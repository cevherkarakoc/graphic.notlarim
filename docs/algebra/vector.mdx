...
() => {
  ...
  return <h4>Sonuç : ( {normalized.join(DOC.separator)} )</h4>;
}
```
...
() => {
  ...
  return <h4>Sonuç : ( {result.join(DOC.separator)} )</h4>;
}
```
...
* Sonuç 0 (sıfır) ise, vektörler birbirin dik açıdadır.
...
() => {
  ...
  const degree = DOC.radiansToDegrees(Math.acos(theta));

  return <h4>Aradaki Açı : {degree}° - {theta} radyan </h4>;
}
```
